...
public union SerializedData
{
    ...
    /// Helper to return an exact type, or wrap `SerializedT`
    public auto get (alias Sym) () const pure nothrow @nogc @trusted
    {
        static if (!serializerInvalidReason!(Sym).length)
            alias T = SerializedT!Sym;
        else
            alias T = Sym;
